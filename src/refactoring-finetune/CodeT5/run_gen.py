...
from evaluator.meteor import compute_meteor_score
from evaluator.rouge import compute_rouge
...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    ...
    if args.task in ['defect']:
        ...
        if args.task == 'summarize':
            ...
            if args.task in ['concode', 'translate', 'refine', 'refactoring']:
                # codebleu = calc_code_bleu.get_codebleu(gold_fn, output_fn, args.lang)
                meteor = compute_meteor_score(gold_fn, output_fn)
                _, rogue_l = compute_rouge(gold_fn,output_fn,4,True)

        result = {
            'em': np.mean(dev_accs) * 100, 
            'bleu': bleu,
            'meteor':round(meteor*100),
            'rogue':round(rogue_l*100)
            }
        if args.task in ['concode', 'refactoring']:
...
def main():
    ...
    args.train_filename, args.dev_filename, args.test_filename = get_filenames(args.data_dir, args.task, args.sub_task, context_folder="no-context")
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                if args.do_eval_bleu:
                    ...
                    dev_meteor, dev_rogue = result['meteor'], result['rogue']
                    logger.info("Meteor: ", dev_meteor)
                    logger.info("Rogue: ", dev_rogue)
                    print("Meteor: ", dev_meteor)
                    print("Rogue: ", dev_rogue)                    
                    if args.task in ['summarize']:
